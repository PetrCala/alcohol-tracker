...
if [[ EXIT_CODE -eq 0 ]]; then
    ...
    echo -e "${RED}Error: Diff found when Github Actions were rebuilt. Did you forget to run \`npm run gh-actions-build\` after a clean install (\`rm -rf node_modules && npm i\`)? Do you need to merge master? Did you try running \`git config --global core.autocrlf false\` then \`npm run gh-actions-build\` again?${NC}"
