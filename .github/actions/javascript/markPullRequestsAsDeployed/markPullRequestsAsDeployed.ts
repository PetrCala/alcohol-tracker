...
async function run() {
    ...
    function getDeployMessage(deployer: string, deployVerb: string, prTitle?: string): string {
        ...
        if (deployVerb === 'Cherry-picked' && !/no ?qa/gi.test(prTitle ?? '')) {
            ...
            message +=
                '\n\nplease QA this PR and check it off on the [deploy checklist](https://github.com/PetrCala/Kiroku/issues?q=is%3Aopen+is%3Aissue+label%3AStagingDeployCash) if it passes.';
        }
