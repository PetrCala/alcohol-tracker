...
import type {
  EdgeInsets,
  useSafeAreaFrame as LibUseSafeAreaFrame,
...
function withSafeAreaInsets(
  WrappedComponent: React.ComponentType<
    WithSafeAreaInsetsProps & {ref: ForwardedRef<unknown>}
  >,
) {
  function WithSafeAreaInsets(
    props: WithSafeAreaInsetsProps & {
  ...
  const WithSafeAreaInsetsWithRef = forwardRef(
    (props: WithSafeAreaInsetsProps, ref: ForwardedRef<unknown>) => (
      <WithSafeAreaInsets
        // eslint-disable-next-line react/jsx-props-no-spreading
        {...props}
        forwardedRef={ref}
...
const SafeAreaConsumer = (props: SafeAreaConsumerProps) =>
  props.children?.(insets);
const SafeAreaInsetsContext: SafeAreaInsetsContextValue = {
  Consumer: SafeAreaConsumer,
};

const useSafeAreaFrame: jest.Mock<ReturnType<typeof LibUseSafeAreaFrame>> =
  jest.fn(() => ({
    x: 0,
    y: 0,
    width: 390,
