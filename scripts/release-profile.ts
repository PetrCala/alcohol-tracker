...
if (argsMap.platform === 'ios') {
  ...
  console.error(
    'Please specify the platform using --platform=ios or --platform=android',
  );
  process.exit(1);
}
...
if (cpuProfiles.length === 0) {
  ...
  console.error(
    'Multiple .cpuprofile files found. Please specify which one to use by placing only one .cpuprofile in the root or specifying the filename as an argument.',
  );
  process.exit(1);
} else {
