...
function computeHorizontalShift(
  ...
  if (tooltipLeftEdge < GUTTER_WIDTH) {
    ...
    return NumberUtils.roundToNearestMultipleOfFour(
      GUTTER_WIDTH - tooltipLeftEdge,
    );
  }
...
function isOverlappingAtTop(
  ...
  const elementAtTargetCenterX = document.elementFromPoint(
    targetCenterX,
    yOffset,
  );
  ...
  if (
    !elementAtTargetCenterX ||
    ('contains' in tooltip && tooltip.contains(elementAtTargetCenterX))
  ) {
    return false;
  }
  ...
  const isOverlappingAtTargetCenterX =
    yOffset > rectAtTargetCenterX.top &&
    yOffset < rectAtTargetCenterX.bottom &&
    yOffset + tooltipTargetHeight > rectAtTargetCenterX.bottom;
...
type GetTooltipStylesStyleUtil = {
  getTooltipStyles: (props: TooltipParams) => TooltipStyles;
};
 ...
 */
const createTooltipStyleUtils: StyleUtilGenerator<
  GetTooltipStylesStyleUtil
> = ({theme, styles}) => ({
  getTooltipStyles: ({
    tooltip,
    currentSize,
    ...
    const isTooltipSizeReady =
      tooltipWidth !== undefined && tooltipHeight !== undefined;
    ...
    if (isTooltipSizeReady) {
      ...
      horizontalShiftPointer =
        horizontalShift > 0
          ? Math.max(
              ...
              -(tooltipWidth / 2) +
                POINTER_WIDTH / 2 +
                variables.componentBorderRadiusSmall,
            )
          : Math.min(
              -horizontalShift,
              tooltipWidth / 2 -
      ...
      rootWrapperLeft =
        xOffset +
        (tooltipTargetWidth / 2 - tooltipWidth / 2) +
        horizontalShift +
        manualShiftHorizontal;
      ...
      pointerWrapperLeft =
        horizontalShiftPointer + (tooltipWidth / 2 - POINTER_WIDTH / 2);
