...
type ApplyUpdatesMockValues = {
  onApplyUpdates:
    | ((updates: DeferredUpdatesDictionary) => Promise<void>)
    | undefined;
};
...
const applyUpdates = jest.fn((updates: DeferredUpdatesDictionary) => {
  ...
  return (
    mockValuesProxy.onApplyUpdates === undefined
      ? Promise.resolve()
      : mockValuesProxy.onApplyUpdates(updates)
  ).then(() =>
    Onyx.set(
      ONYXKEYS.ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT,
      Math.max(lastUpdateIDAppliedToClient, lastUpdateIdFromUpdates),
    ),
  );
});
