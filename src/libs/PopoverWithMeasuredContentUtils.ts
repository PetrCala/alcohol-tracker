...
function computeHorizontalShift(
  ...
  if (popoverRightEdge > windowWidth - variables.gutterWidth) {
    ...
    return roundToNearestMultipleOfFour(
      windowWidth - variables.gutterWidth - popoverRightEdge,
    );
  }
...
function computeVerticalShift(
  ...
  if (anchorTopEdge < 0) {
    ...
    canSwitchPosition =
      popoverBottomEdge + menuHeight + anchorHeight <= windowHeight;
    return roundToNearestMultipleOfFour(
      shoudSwitchPositionIfOverflow && canSwitchPosition
        ? menuHeight + anchorHeight
        : 0 - anchorTopEdge,
    );
  }
  ...
  if (popoverBottomEdge > windowHeight) {
    ...
    return roundToNearestMultipleOfFour(
      shoudSwitchPositionIfOverflow && canSwitchPosition
        ? -(menuHeight + anchorHeight)
        : windowHeight - popoverBottomEdge,
    );
  }
...
const PopoverWithMeasuredContentUtils = {
