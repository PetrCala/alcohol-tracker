...
Onyx.connect({
  ...
  callback: val => {
    ...
    emailToPersonalDetailsCache = personalDetails.reduce(
      (acc: Record<string, PersonalDetails>, detail) => {
        if (detail?.login) {
...
function getDisplayNameOrDefault(
  ...
  //   if (
  //     displayName === passedPersonalDetails?.login &&
  //     Str.isSMSLogin(passedPersonalDetails?.login)
  //   ) {
...
function createDisplayName(
  ...
  passedPersonalDetails:
    | Pick<PersonalDetails, 'firstName' | 'lastName'>
    | OnyxEntry<PersonalDetails>,
): string {
...
function extractFirstAndLastNameFromAvailableDetails({
  ...
  firstName,
  lastName,
}: CurrentUserPersonalDetails): FirstAndLastName {
