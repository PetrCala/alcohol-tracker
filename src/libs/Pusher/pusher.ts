...
type EventData<EventName extends string> =
  EventName extends keyof PusherEventMap
    ? PusherEventMap[EventName]
    : OnyxUpdatesFromServer;

type EventCallbackError = {
  type: ValueOf<typeof CONST.ERROR>;
  data: {code: number};
};
...
type SocketEventCallback = (
  ...
  data?: States | EventCallbackError,
) => void;
...
type PusherSubscribtionErrorData = {
  type?: string;
  error?: string;
  status?: string;
};
 ...
 */
function callSocketEventCallbacks(
  eventName: SocketEventName,
  data?: EventCallbackError | States,
) {
...
function bindEventToChannel<EventName extends PusherEventName>(
  ...
  const callback = (eventData: EventData<EventName>) => {
    ...
    try {
      ...
      Log.alert('[Pusher] Unable to parse single JSON event data from Pusher', {
        error: err,
        eventData,
      });
      return;
    ...
    if (
      ...
      chunkedEvent.chunks.length === Object.keys(chunkedEvent.chunks).length
    ) {
      try {
    ...
    InteractionManager.runAfterInteractions(() => {
      ...
      if (!channel?.subscribed) {
        ...
        channel.bind(
          ...
          (data: PusherSubscribtionErrorData = {}) => {
 ...
 */
function sendEvent<EventName extends PusherEventName>(
  channelName: string,
  eventName: EventName,
  payload: EventData<EventName>,
) {
  ...
  if (shouldForceOffline) {
    Log.info(
      '[Pusher] Ignoring a Send event because shouldForceOffline = true',
    );
    return;
  }
