...
type SearchPageProps = StackScreenProps<BottomTabNavigatorParamList>;
//   typeof SCREENS.SEARCH.BOTTOM_TAB
...
function setupCustomAndroidBackHandler() {
  const onBackPress = () => {
    ...
    const bottomTabRoute = rootState?.routes?.find(
    ...
    const isLastScreenOnStack =
      bottomTabRoutes.length === 1 && rootState?.routes?.length === 1;
    ...
    // if (
    //   bottomTabRoutes[bottomTabRoutes.length - 1].name ===
    //     SCREENS.SEARCH.BOTTOM_TAB &&
    //   focusedRoute?.name === SCREENS.SEARCH.CENTRAL_PANE
    // ) {
    //   navigationRef.dispatch({
    //     ...StackActions.pop(),
    //     target: bottomTabRoute?.state?.key,
    //   });
    //   navigationRef.dispatch({...StackActions.pop()});

    //   const centralPaneRouteAfterPop = getTopmostCentralPaneRoute({
    //     routes: [rootState?.routes?.at(-2)],
    ...
    //     (!centralPaneRouteAfterPop ||
    //       centralPaneRouteAfterPop.name !== SCREENS.SEARCH.CENTRAL_PANE)
    //   ) {
    //     const searchParams = bottomTabRoutes[bottomTabRoutes.length - 2]
    //       .params as SearchPageProps['route']['params'];
    //     navigationRef.dispatch({
    //       ...StackActions.push(SCREENS.SEARCH.CENTRAL_PANE, searchParams),
    //     });
    //   }
    ...
    // if (
    //   bottomTabRoutes &&
    //   bottomTabRoutes?.length >= 2 &&
    //   bottomTabRoutes[bottomTabRoutes.length - 2].name ===
    //     SCREENS.SEARCH.BOTTOM_TAB &&
