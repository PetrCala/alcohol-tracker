...
const getTopMostReportIDFromRHP = (state: State): string => {
  ...
  if (
    topmostRoute?.params &&
    'reportID' in topmostRoute.params &&
    typeof topmostRoute.params.reportID === 'string'
  ) {
...
// function getAlreadyOpenedSettingsScreen(
//   rootState?: State,
// ): keyof AuthScreensParamList | undefined {
//   if (!rootState) {
//     return undefined;
//   }
...
//   const alreadyOpenedSettingsScreen = rootState.routes
//     .filter(item =>
//       TAB_TO_CENTRAL_PANE_MAPPING[SCREENS.SETTINGS.ROOT].includes(
//         item.name as CentralPaneName,
//       ),
//     )
//     .at(-1);
...
function getMatchingCentralPaneRouteForState(
  ...
  const centralPaneName =
    TAB_TO_CENTRAL_PANE_MAPPING[topmostBottomTabRoute.name][0];
  ...
  //     const screen = centralPaneName;
  //     return {
  //       name: screen as CentralPaneName,
  ...
  if (topmostBottomTabRoute.name === SCREENS.HOME) {
    return {
      ...
      params: {reportID: getTopMostReportIDFromRHP(state)},
    };
  }
