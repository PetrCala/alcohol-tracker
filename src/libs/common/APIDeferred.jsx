...
export default function APIDeferred(promise, extractedProperty) {
  ...
  if (!promise) {
    throw new Error(
      'Cannot instantiate APIDeferred without a Deferred promise',
    );
  }
  ...
  function handleDone(response) {
    ...
    const requestedPropertyPresent =
      propertyRequested &&
      response &&
      !_.isUndefined(response[extractedPropertyName]);
    const propertyRequestedButMissing =
      propertyRequested && !requestedPropertyPresent;
    ...
    if (jsonCode === 200 && (!propertyRequested || requestedPropertyPresent)) {
      ...
      returnedData =
        propertyRequested && requestedPropertyPresent
          ? response[extractedPropertyName]
          : response;
      ...
      if (propertyRequestedButMissing) {
        throw new Error(
          `Requested property " ${extractedPropertyName}" missing from response`,
        );
      }
      if (!response) {
