...
function LegalNameScreen(props) {
  const styles = useThemeStyles();
  usePrivatePersonalDetails();
  const legalFirstName = lodashGet(
    props.privatePersonalDetails,
    'legalFirstName',
    '',
  );
  const legalLastName = lodashGet(
    props.privatePersonalDetails,
    'legalLastName',
    '',
  );
  const isLoadingPersonalDetails = lodashGet(
    props.privatePersonalDetails,
    'isLoading',
    true,
  );
  ...
  const validate = useCallback(values => {
    ...
    if (!ValidationUtils.isValidLegalName(values.legalFirstName)) {
      ...
      ErrorUtils.addErrorMessage(errors, 'legalFirstName', [
        ...
        {
          length: values.legalFirstName.length,
    ...
    if (!ValidationUtils.isValidLegalName(values.legalLastName)) {
      ...
      ErrorUtils.addErrorMessage(errors, 'legalLastName', [
        'common.error.characterLimitExceedCounter',
        {
          length: values.legalLastName.length,
          limit: CONST.TITLE_CHARACTER_LIMIT,
        },
      ]);
    }
  ...
  return (
    <ScreenWrapper
      ...
      testID={LegalNameScreen.displayName}>
...
LegalNameScreen.propTypes = propTypes;
LegalNameScreen.defaultProps = defaultProps;
LegalNameScreen.displayName = 'LegalNameScreen';
