...
function ReceiptCell({transactionItem}: TransactionCellProps) {
  ...
  const backgroundStyles = transactionItem.isSelected
    ? StyleUtils.getBackgroundColorStyle(theme.buttonHoveredBG)
    : StyleUtils.getBackgroundColorStyle(theme.border);

  const isViewAction =
    transactionItem.action === CONST.SEARCH.ACTION_TYPES.VIEW;
    ...
    <View
      ...
      <ReceiptImage
        source={tryResolveUrlFromApiRoot(
          transactionItem?.receipt?.source ?? '',
        )}
        ...
        isAuthTokenRequired
        fallbackIcon={
          canModifyReceipt ? Expensicons.ReceiptPlus : Expensicons.ReceiptSlash
        }
        fallbackIconSize={20}
        fallbackIconColor={theme.icon}
        fallbackIconBackground={
          transactionItem.isSelected ? theme.buttonHoveredBG : undefined
        }
        iconSize="x-small"
      />
    </View>
...
function DateCell({
  ...
  const date = DateUtils.formatWithUTCTimeZone(
    created,
    DateUtils.doesDateBelongToAPastYear(created)
      ? CONST.DATE.MONTH_DAY_YEAR_ABBR_FORMAT
      : CONST.DATE.MONTH_DAY_ABBR_FORMAT,
  );
  ...
  return (
    <TextWithTooltip
      ...
      text={date}
      style={[
        styles.lineHeightLarge,
        styles.pre,
        styles.justifyContentCenter,
        isLargeScreenWidth
          ? undefined
          : [styles.textMicro, styles.textSupporting],
      ]}
    />
  );
}
...
function MerchantCell({
  ...
  if (!merchantOrDescriptionToDisplay && !isLargeScreenWidth) {
    merchantOrDescriptionToDisplay = Parser.htmlToText(
      Parser.replace(description),
    );
  }
  let merchant = transactionItem.shouldShowMerchant
    ? merchantOrDescriptionToDisplay
    : Parser.htmlToText(Parser.replace(description));

  if (
    TransactionUtils.hasReceipt(transactionItem) &&
    TransactionUtils.isReceiptBeingScanned(transactionItem) &&
  ...
  return (
    <TextWithTooltip
      ...
      style={[
        isLargeScreenWidth ? styles.lineHeightLarge : styles.lh20,
        styles.pre,
        styles.justifyContentCenter,
      ]}
    />
...
function TotalCell({
  ...
  let amount = CurrencyUtils.convertToDisplayString(
    transactionItem.formattedTotal,
    currency,
  );
...
function TaxCell({transactionItem, showTooltip}: TransactionCellProps) {
  ...
  const isFromExpenseReport =
    transactionItem.reportType === CONST.REPORT.TYPE.EXPENSE;
  const taxAmount = TransactionUtils.getTaxAmount(
    transactionItem,
    isFromExpenseReport,
  );
  const currency = TransactionUtils.getCurrency(transactionItem);
      ...
      <View style={containerStyle}>
        ...
        <View
          ...
          <View
            style={[
              styles.flex2,
              !item.category && styles.justifyContentCenter,
              styles.gap1,
            ]}>
            <MerchantCell
              transactionItem={item}
            ...
            {!!item.category && (
              <View
                style={[styles.flexRow, styles.flex1, styles.alignItemsEnd]}>
                <CategoryCell
                  isLargeScreenWidth={false}
                  showTooltip={showTooltip}
                  transactionItem={item}
    ...
    <View
      ...
      <View
        style={[
          ...
          styles.alignItemsCenter,
          styles.gap3,
          canSelectMultiple && styles.pl4,
        ]}>
        <View
          style={[
            StyleUtils.getSearchTableColumnStyles(
        ...
        <View
          ...
          <UserInfoCell participant={item.to} displayName={item.formattedTo} />
        </View>
