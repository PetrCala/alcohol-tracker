...
type PopoverWithMeasuredContentProps = Omit<
  PopoverProps,
  'anchorPosition' | keyof WindowDimensionsProps
> & {
...
function PopoverWithMeasuredContent({
  ...
  const [isContentMeasured, setIsContentMeasured] = useState(
  ...
  if (!isPopoverVisible && isVisible) {
    ...
    setIsContentMeasured(
      popoverDimensions.width > 0 && popoverDimensions.height > 0,
    );
  ...
  const horizontalShift =
    PopoverWithMeasuredContentUtils.computeHorizontalShift(
  ...
  const shiftedAnchorPosition = {
    ...
    bottom:
      windowHeight -
      (adjustedAnchorPosition.top + popoverHeight) -
      verticalShift,
  };
  ...
  */
    <View style={styles.invisiblePopover} onLayout={measurePopover}>
      {children}
    </View>
  );
}
