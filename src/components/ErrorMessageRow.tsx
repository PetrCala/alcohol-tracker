...
function ErrorMessageRow({
  errors,
  errorRowStyles,
  onClose,
  canDismissError = true,
}: ErrorMessageRowProps) {
  // Some errors have a null message. This is used to apply opacity only and to avoid showing redundant messages.
  const errorEntries = Object.entries(errors ?? {});
  const filteredErrorEntries = errorEntries.filter(
    (errorEntry): errorEntry is [string, string] => errorEntry[1] !== null,
  );
  const errorMessages = mapValues(
    Object.fromEntries(filteredErrorEntries),
    error => error,
  );
