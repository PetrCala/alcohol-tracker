...
function YearPickerModal({
  ...
  const {sections, headerMessage} = useMemo(() => {
    const yearsList =
      searchText === ''
        ? years
        : years.filter(year => year.text?.includes(searchText));
    return {
      headerMessage: !yearsList.length
        ? translate('common.noResultsFound')
        : '',
      sections: [
        {data: yearsList.sort((a, b) => b.value - a.value), indexOffset: 0},
      ],
    };
    ...
    <Modal
      ...
      <ScreenWrapper
        ...
        <HeaderWithBackButton
          title={translate('yearPickerScreen.year')}
          onBackButtonPress={onClose}
        />
        <SelectionList
          textInputLabel={translate('yearPickerScreen.selectYear')}
