...
function mergeRefs(
  ...args: Array<MutableRefObject<FlatList> | ForwardedRef<FlatList> | null>
) {
...
function MVCPFlatList<TItem>(
  ...
  const getScrollOffset = useCallback((): number => {
    ...
    return horizontal
      ? getScrollableNode(scrollRef.current)?.scrollLeft ?? 0
      : getScrollableNode(scrollRef.current)?.scrollTop ?? 0;
  }, [horizontal]);

  const getContentView = useCallback(
    () => getScrollableNode(scrollRef.current)?.childNodes[0],
    [],
  );
  ...
  const adjustForMaintainVisibleContentPosition = useCallback(
    (animated = true) => {
      ...
      if (Math.abs(delta) > (IS_MOBILE_SAFARI ? 100 : 0.5)) {
        ...
        if (
          mvcpAutoscrollToTopThresholdRef.current != null &&
          scrollOffset <= mvcpAutoscrollToTopThresholdRef.current
        ) {
      ...
      mutations.forEach(mutation => {
        ...
        mutation.addedNodes.forEach(node => {
          if (
            ...
            !(node as HTMLElement).querySelector('#composer')
          ) {
            return;
          }
