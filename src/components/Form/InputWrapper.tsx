...
type TextInputBasedComponents = [ComponentType<BaseTextInputProps>];

const textInputBasedComponents: TextInputBasedComponents = [TextInput];
...
function computeComponentSpecificRegistrationParams({
  ...
  if (textInputBasedComponents.includes(InputComponent)) {
    ...
    const shouldReallySubmitForm = isEffectivelyMultiline
      ? !!shouldSubmitForm && canUseHardwareKeyboard
      : true;
    ...
    return {
      ...
      blurOnSubmit:
        (isEffectivelyMultiline && shouldReallySubmitForm) || blurOnSubmit,
...
type InputWrapperProps<
  TInput extends ValidInputs,
  TValue extends ValueTypeKey = ValueTypeKey,
> = ComponentPropsWithoutRef<TInput> &
...
function InputWrapper<TInput extends ValidInputs, TValue extends ValueTypeKey>(
  props: InputWrapperProps<TInput, TValue>,
  ...
  const {shouldSetTouchedOnBlurOnly, blurOnSubmit, shouldSubmitForm} =
