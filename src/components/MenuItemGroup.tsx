...
type MenuItemGroupContextProps = {
  isExecuting: boolean;
  singleExecution: <T extends unknown[]>(
...
const MenuItemGroupContext = createContext<
  MenuItemGroupContextProps | undefined
>(undefined);
  ...
  const value = useMemo(
    () =>
      shouldUseSingleExecution
        ? {isExecuting, singleExecution, waitForNavigate}
        : undefined,
    [shouldUseSingleExecution, isExecuting, singleExecution, waitForNavigate],
  );

  return (
    <MenuItemGroupContext.Provider value={value}>
      {children}
