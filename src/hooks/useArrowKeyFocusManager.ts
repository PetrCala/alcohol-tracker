...
export default function useArrowKeyFocusManager({
  ...
  const arrowUpCallback = useCallback(() => {
    ...
    setFocusedIndex(actualIndex => {
      const currentFocusedIndex =
        actualIndex > 0 ? actualIndex - (itemsPerRow ?? 1) : nextIndex;
    ...
    maxIndex,
    isFocused,
    disableCyclicTraversal,
    itemsPerRow,
    disabledIndexes,
    allowNegativeIndexes,
  ]);
  ...
  const arrowDownCallback = useCallback(() => {
    ...
    setFocusedIndex(actualIndex => {
      ...
      if (actualIndex === -1) {
        ...
        currentFocusedIndex =
          actualIndex < maxIndex ? actualIndex + (itemsPerRow ?? 1) : nextIndex;
      }
  ...
  const arrowRightCallback = useCallback(() => {
    ...
    setFocusedIndex(actualIndex => {
      const currentFocusedIndex =
        actualIndex < maxIndex ? actualIndex + 1 : nextIndex;

      let newFocusedIndex = currentFocusedIndex;

      while (disabledIndexes.includes(newFocusedIndex)) {
        newFocusedIndex =
          newFocusedIndex < maxIndex ? newFocusedIndex + 1 : nextIndex;
