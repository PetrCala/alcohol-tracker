...
export default function useResponsiveLayout(): ResponsiveLayoutResult {
  ...
  const isExtraSmallScreenHeight =
    windowHeight <= variables.extraSmallMobileResponsiveHeightBreakpoint;
  ...
  const onboardingIsMediumOrLargerScreenWidth =
  ...
  const isDisplayedInNarrowModalNavigator = useMemo(
    () =>
      !!currentNavigator?.getParent?.(
        NAVIGATORS.RIGHT_MODAL_NAVIGATOR as unknown as undefined,
      ) ||
      !!currentNavigator?.getParent?.(
        NAVIGATORS.LEFT_MODAL_NAVIGATOR as unknown as undefined,
      ),
    [currentNavigator],
  );
