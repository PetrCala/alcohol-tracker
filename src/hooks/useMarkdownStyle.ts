...
function useMarkdownStyle(
  ...
  excludeStyles: Array<keyof MarkdownStyle> = defaultEmptyArray,
): MarkdownStyle {
  const theme = useTheme();
  const hasMessageOnlyEmojis = false; // was message != null && message.length > 0 && containsOnlyEmojis(message);
  const emojiFontSize = hasMessageOnlyEmojis
    ? variables.fontSizeOnlyEmojis
      ...
      excludeStyles.forEach(key => {
        ...
        if (style) {
          Object.keys(style).forEach(styleKey => {
            style[styleKey] =
              nonStylingDefaultValues[styleKey] ?? style[styleKey];
          });
        }
      });
    }
