...
class ExtendedAct extends Act {
  ...
  async runEvent(event: string, opts?: ExtendedActOpts): Promise<Step[]> {
    ...
    if (mockJobs) {
      await this.handleJobMocking(workflow => workflow.events.includes(event), {
        mockJobs,
        workflowFile: vanillaOpts.workflowFile,
        cwd: vanillaOpts.cwd,
      });
    }
  ...
  async handleJobMocking(
    filter: (workflow: Workflow) => boolean,
    opts: ExtendedActOpts,
  ): Promise<void[]> {
    ...
    if (opts.workflowFile) {
      ...
      const availableWorkflows = await this.list(
        undefined,
        opts.cwd,
        opts.workflowFile,
      );
      workflowFiles = availableWorkflows
        .filter(filter)
        .map((workflow: Workflow) => workflow.workflowFile);
    }
